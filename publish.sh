...
echo "${NC}"
if [ "$QUALITY_ONLY" = true ]; then
    echo "${BOLD}Starting Jeff Quality Checks...${NC}"
else
...
sed -i.bak "s/__version__ = \".*\"/__version__ = \"${VERSION}\"/" src/jeff/__init__.py && rm src/jeff/__init__.py.bak
...
run_command "git tag jeff-${VERSION}" "Creating git tag"
...
echo "${GREEN}${BOLD}═══════════════════════════════════════════════════════════════${NC}"
echo "${GREEN}Jeff v${VERSION} has been successfully published and tagged!${NC}"
echo ""
